...
import { X, Sparkles, Clock, Gift, Star, Trophy, Truck, Tag, Zap } from "lucide-react";
...
export default function SpecialEventPopup({
  ...
  const getColorValue = (color: string | { hex: string } | undefined): string => {
  ...
  const addProductWithDiscount = (discountProduct: any) => {
    ...
    if (discountProduct.discountType === 'percentage') {
      const discountAmount = (product.price * discountProduct.discountValue) / 100;
      ...
      discountPercentage = Math.round((discountProduct.discountValue / product.price) * 100);
    } else if (discountProduct.discountType === 'final_price') {
      discountedPrice = discountProduct.discountValue;
      discountPercentage = Math.round(((product.price - discountProduct.discountValue) / product.price) * 100);
    }
    ...
    const cartItem: CartItem = {
      ...
      gallery: discountProduct.customImage ? [discountProduct.customImage] : product.gallery || [],
      quantity: 1,
      inStock: true,
    };
    ...
    alert(`Added ${product.title} to cart with ${discountPercentage}% discount!`);
  };
  ...
  console.log('SpecialEventPopup: Main image data:', safeEvent.advancedSettings.mainImage);
  console.log('SpecialEventPopup: Main image URL:', safeEvent.advancedSettings.mainImage?.asset?.url);

  const primaryFeatures = safeEvent.eventFeatures.filter(f => f.isPrimary) || [];
  const secondaryFeatures = safeEvent.eventFeatures.filter(f => !f.isPrimary) || [];
    ...
    >
      ...
      >
          ...
          >
            ...
            {safeEvent.advancedSettings?.backgroundPattern && safeEvent.advancedSettings.backgroundPattern !== 'none' && (
              <div className="absolute inset-0 rounded-3xl overflow-hidden pointer-events-none">
                {(() => {
                  ...
                  const patternEmojis = emojis[pattern as keyof typeof emojis] || [];
            ...
            <div className="relative p-4 sm:p-6 lg:p-8">
                ...
                <div className="mb-4 sm:mb-6 -mx-4 sm:-mx-6 md:-mx-8 lg:-mx-10 -mt-4 sm:-mt-6 md:-mt-8 lg:-mt-10">
                  <div className="relative h-24 sm:h-32 md:h-40 lg:h-48 xl:h-56 rounded-t-2xl sm:rounded-t-3xl overflow-hidden shadow-xl sm:shadow-2xl">
                    <Image
                      ...
                      alt={(() => {
                        switch (language) {
                          case 'fr': return safeEvent.eventName_fr || safeEvent.eventName;
                          case 'ar': return safeEvent.eventName_ar || safeEvent.eventName;
                          default: return safeEvent.eventName;
                        }
                      })()}
                      fill
                    ...
                    <div className="absolute bottom-1 left-1 right-1 sm:bottom-2 sm:left-2 sm:right-2 md:bottom-4 md:left-4 md:right-4" dir={language === "ar" ? "rtl":'ltl'}> 
                      <div className="bg-black/30 backdrop-blur-sm rounded-lg sm:rounded-xl p-1 sm:p-2 md:p-4 border border-white/20">
                        <h3 className="text-white text-xs sm:text-xs md:text-sm font-bold mb-0.5 sm:mb-1 drop-shadow-lg uppercase leading-tight">
                          {(() => {
                            switch (language) {
                              case 'fr': return (safeEvent.headline_fr || safeEvent.headline).toUpperCase();
                              case 'ar': return (safeEvent.headline_ar || safeEvent.headline).toUpperCase();
                              default: return safeEvent.headline.toUpperCase();
                            }
                          })()}
                        </h3>
                        <p className="text-white/90 text-xs drop-shadow-md hidden md:block">
                          {(() => {
                            switch (language) {
                              case 'fr': return safeEvent.description_fr?.slice(0, 50) || safeEvent.description?.slice(0, 50);
                              case 'ar': return safeEvent.description_ar?.slice(0, 50) || safeEvent.description?.slice(0, 50);
                              default: return safeEvent.description?.slice(0, 50);
                            }
                          })()}...
                        </p>
                      </div>
              ...
              <div className="text-center mb-4 sm:mb-6">
                ...
                <p
                  className="text-gray-600 dark:text-gray-400 text-xs sm:text-xs md:text-sm lg:text-base xl:text-lg px-2 sm:px-4 mb-2 sm:mb-3 max-w-2xl mx-auto leading-relaxed"
                  dir={language === 'ar' ? 'rtl' : 'ltr'}
                >
                  {(() => {
                    switch (language) {
                      case 'fr': return safeEvent.description_fr || safeEvent.description;
                      case 'ar': return safeEvent.description_ar || safeEvent.description;
                      default: return safeEvent.description;
                    }
                  })()}
                </p>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 mb-4">
                {safeEvent.eventTemplate && safeEvent.eventTemplate !== 'custom' && (
                ...
                {safeEvent.advancedSettings?.priority && safeEvent.advancedSettings.priority > 5 && (
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span
                    ...
                    style={{ color: safeEvent.themeColors.primaryColor }}
                  >
                    {t('eventEndsIn')} {Math.floor(timeRemaining / 86400)} {t('days')}
                  </span>
                </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          ...
                          style={{ backgroundColor: safeEvent.themeColors.primaryColor }}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-gray-900 dark:text-white text-xs sm:text-sm mb-1">
                            {(() => {
                              switch (language) {
                                case 'fr': return feature.featureTitle_fr || feature.featureTitle;
                                case 'ar': return feature.featureTitle_ar || feature.featureTitle;
                                default: return feature.featureTitle;
                              }
                            })()}
                          </h3>
                          {feature.featureDescription && (
                            <p className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm">
                              {(() => {
                                switch (language) {
                                  case 'fr': return feature.featureDescription_fr || feature.featureDescription;
                                  case 'ar': return feature.featureDescription_ar || feature.featureDescription;
                                  default: return feature.featureDescription;
                                }
                              })()}
                            </p>
                          )}
                          {feature.featureValue && (
                            <span
                              ...
                              style={{
                                backgroundColor: safeEvent.themeColors.secondaryColor,
                                color: 'white',
                              }}
                            >
                              {feature.featureValue}
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:gap-3">
                      {secondaryFeatures.map((feature) => (
                        <div
                          ...
                          style={{ borderColor: `${safeEvent.themeColors.primaryColor}20` }}
                        >
                          <div
                          ...
                          {feature.featureValue && (
                            <span className="text-xs font-medium" style={{ color: safeEvent.themeColors.primaryColor }}>
                              {feature.featureValue}
                            </span>
                          )}
                        </div>
              ...
              {safeEvent.discountProducts && safeEvent.discountProducts.length > 0 && (
                <div className="mb-4">
                  <div className="text-center mb-3 sm:mb-4">
                    <h3 className="font-semibold text-gray-900 dark:text-white text-xs sm:text-sm md:text-base mb-1">
                  ...
                  <div className="space-y-3">
                    {safeEvent.discountProducts.map((discountProduct) => {
                      ...
                      const firstImage = discountProduct.customImage?.asset?.url ||
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            {/* Product Info */}
                            <div className="flex-1 min-w-0">
                              <h4 className="font-semibold text-gray-900 dark:text-white text-xs sm:text-xs md:text-sm mb-1 line-clamp-2">
                                {product.title}
                              </h4>
                              ...
                              <div className="flex items-center gap-1 sm:gap-2 mb-2">
                                ...
                                {discountPercentage > 0 && (
                                  <span
                                    className="text-xs font-semibold px-1 sm:px-2 py-0.5 sm:py-1 rounded-full text-white"
                                    style={{ backgroundColor: safeEvent.themeColors.primaryColor }}
                            ...
                            {/* Add to Cart Button */}
                            <button
                              onClick={() => addProductWithDiscount(discountProduct)}
                              className="px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl font-semibold text-xs sm:text-sm text-white shadow-lg flex-shrink-0"
                              style={{
